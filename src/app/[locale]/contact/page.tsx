          ...
          <div className="border border-border rounded-lg bg-card p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">UTC+0 · Afrique de l'Ouest</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded border border-border bg-muted flex items-center justify-center flex-shrink-0">
          ...
          {status === "success" ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <Button variant="outline" size="sm"
                onClick={() => { setStatus("idle"); setForm({ name:"", email:"", subject:"", service:"", message:"" }); }}>
                Nouveau message
              </Button>
            </motion.div>
          ) : (
            <form onSubmit={handleSubmit} className="border border-border rounded-lg bg-card p-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">{t("name")} *</label>
                  <Input value={form.name} onChange={f("name")} required minLength={2} placeholder="Jean Dupont" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">{t("email")} *</label>
                  <Input type="email" value={form.email} onChange={f("email")} required placeholder="vous@exemple.com" />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.service} onChange={f("service")} placeholder="WordPress, Flutter, React…" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">{t("message")} *</label>
                <Textarea value={form.message} onChange={f("message")} required rows={5} minLength={10} maxLength={2000} />
              </div>
              {status === "error" && (
                <div className="flex items-center gap-2 text-xs text-destructive bg-destructive/10 border border-destructive/20 rounded px-3 py-2">
                  <AlertCircle className="h-3.5 w-3.5 flex-shrink-0" /> {t("error")}
                </div>
              )}
              <Button type="submit" disabled={status === "sending"} className="w-full gap-2 hover: bg-foreground">
